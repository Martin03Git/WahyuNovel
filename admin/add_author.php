<?php
    ...
    // Tentukan tujuan berdasarkan parameter 'location'
    if (isset($_GET['location'])) {
        $location = $_GET['location'];
        if ($location === 'add') {
            $tujuan = 'add_novel.php';
        } else if ($location === 'edit') {
            $id_novel = $_GET['id'];
            $tujuan = "edit_novel.php?id=$id_novel";
        }
    } else {
        $tujuan = "manage_author.php";
    }

    // Memproses form saat disubmit untuk menambah author baru
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        $nama_author = $_POST['nama_author'];

        // Query untuk menambahkan author ke dalam tabel author
        $sql = "INSERT INTO author (nama_author) VALUES ('$nama_author')";
        if ($conn->query($sql) === TRUE) {
            // Redirect sesuai tujuan
            if ($_GET['location'] == 'add') {
                header('Location: add_novel.php');
            } elseif ($_GET['location'] == 'edit') {
                header("location: edit_novel.php?id=$id_novel");
            } else {
  ...
  <div class="container-fluid d-flex">
    ...
    <div class="sidebar d-flex flex-column flex-shrink-0 p-3 text-bg-dark" style="width: 280px;">
      ...
      <ul class="nav nav-pills flex-column mb-auto">
        <li class="nav-item ">
          <a href="./index.php" class="nav-link active d-flex align-items-center gap-3" aria-current="page">
    ...
    <div class="main d-flex flex-grow-1 p-3 flex-column">
      <h2 class="text-center mb-3 align-self-strech"><b>Manage Author Novel</b></h2>


      <!-- Form untuk menambah author baru -->
      <form method="POST" action="" class="gap-1 mx-auto">
        <div class="form-group mb-2">
          <label for="nama_author" class="mb-1"><b>Nama Author:</b></label>
